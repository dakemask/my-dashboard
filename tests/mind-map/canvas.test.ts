...
import type { MindMapDocument, MindMapNode } from "../../src/mind-map/domain";
import {
  IDENTITY_VIEWPORT,
  MindMapCanvas,
...
describe("MindMapCanvas keyed rendering", () => {
  it("preserves keyed node, arrow, and textarea elements across data renders", () => {
    ...
    expect([...fixture.canvas.element.querySelectorAll<SVGGElement>(".mind-map-canvas__node")]
  ...
  it("updates only affected keyed geometry during a move and releases capture/auto-pan", () => {
    ...
    const moveHit = movedNode.querySelector<SVGRectElement>(".mind-map-canvas__node-move-hit")!;
  ...
  it("keeps a selected group intact for Ctrl+drag and moves the whole group", () => {
    ...
    const moveHit = nodeGroup(fixture.canvas, "a")
      .querySelector<SVGRectElement>(".mind-map-canvas__node-move-hit")!;
  ...
  it("cancels geometry, overlays, and auto-pan when pointer capture is lost", () => {
    ...
    expect(fixture.canvas.element.querySelector(".mind-map-canvas__marquee")).not.toBeNull();
    dispatchPointer(fixture.canvas.element, "lostpointercapture", {
      pointerId: 12,
      clientX: 210,
      clientY: 170,
    });
    expect(fixture.canvas.element.querySelector(".mind-map-canvas__marquee")).toBeNull();
    expect(fixture.scheduler.size).toBe(0);
  });
