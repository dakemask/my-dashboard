...
- 所有 UI 的内联文字编辑必须“无感”：进入、输入、校验失败、提交或取消编辑时，文字的字体、字号、字重、行高、换行规则和对齐方式，以及编辑区域的宽高、内外边距、周边占位和父级结构，都应与只读态保持基本一致，不得因替换为 `input`/`textarea` 而发生布局跳动。优先让同一输入控件在只读态与编辑态间切换；必须替换元素时，两态共用排版规则，并在显示前按内容同步 textarea 高度。选中态、状态标记等会影响可用空间的元素在编辑时必须保留原占位。焦点、校验和编辑状态只能使用不改变盒模型的颜色、背景、`outline`、内描边或阴影表达，不能通过新增边框、改变 padding、插入操作行等方式挤压或重排内容。
