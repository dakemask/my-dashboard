...
- 未编辑时点击正文不会显示卡片或正文外框。进入编辑后，仅在文字编辑区域内部显示带圆角的 1px 灰色细线；焦点位于正文编辑区时细线变为蓝色，焦点离开编辑区后恢复灰色。
- 编辑在对应卡片内无感原地进行。每条想法按 ID 复用同一个正文 textarea：只读态设置 `readOnly`，编辑态只解除只读并聚焦，不替换正文控件；textarea 始终按内容自动增高。只读态的搜索命中由与 textarea 完全重叠、不可交互且 `aria-hidden` 的镜像层绘制，textarea 仍是唯一可访问正文控件。进入或退出编辑不能改变正文的字体、字号、行高、换行、尺寸或卡片操作区宽度。编辑期间只提供“保存修改”和“取消”。正文未实际变化时退出编辑但不创建版本；有效变化会追加新版本并自动保存到本机。
- 取消已经修改的编辑草稿前必须确认。
