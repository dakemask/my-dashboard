...
没有账户注册表时首页直接进入本地模式。首页以 `dashboardModuleCatalog` 作为模块入口、持久化 definition 和错误模块名的唯一来源；页面路由 slug 与数据 `moduleId` 可以不同，例如 `mind-map` 与 `mind-maps`。

添加首个账户时，首页先对 catalog 中全部持久化模块完成只读 preflight，复用 `modulePayload` 的 clone、迁移、校验、content key 与 hash 管线；当前 schema 的本机 envelope 若 hash 不一致，必须在任何远端写入或账户注册前拒绝。若整个仪表盘的本地与云端均有业务数据，同一个账户设置 dialog 让用户统一选择本地覆盖云端或云端覆盖本地，随后才按顺序逐模块复用标准覆盖流程。

绑定或账户注册失败时，要尝试删除未注册目标 profile 下的全部临时模块库，`local` profile 和已有账户保持不变。`cloud-wins` 不改云端；`local-wins` 一旦开始则可能已经更新前面的云端模块，跨模块没有回滚事务，错误反馈必须说明保持“本地覆盖云端”方向可安全重试，不得承诺远端未变化。全部绑定并保存账户注册表后才清理 `local` profile。添加后不再提供本地模式。后续账户只建立独立 profile，首次打开模块时按普通账户初始化从其云端拉取。
