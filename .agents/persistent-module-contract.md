...
本文供所有需要保存业务数据的模块长期使用。它解释模块怎样划分状态、怎样调用 Shared SDK，以及 SDK 对模块承诺的可观察行为；IndexedDB、GitHub 提交、轮询器和锁的内部实现见 [Shared 维护](./shared-maintenance.md)。

维护已有模块时，只需再阅读该模块自己的长期文档。只有首次接入或重做 SDK 接线时，才阅读 [新持久化模块接入指南](./new-persistent-module-guide.md)。
...
- 测试和夹具必须注入模拟 GitHub 边界，不得访问真实私人仓库。

通常还应：

- 让 event 对应用户理解的语义动作，并在模块文档中写清提交时机；
- 根据典型 event 大小和实际撤销需要选择容量，而不是统一套用固定值；
- 在 `project` 后清理不再可靠的实时引用；
- 把保存、上传、拉取和冲突方向做成明确、可重试的用户动作；
- 让模块测试聚焦自己的 payload、event、hooks 和 UI 接线，不重复测试 Shared 内部算法。
