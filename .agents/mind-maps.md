...
按下右下手柄后，画布在本次交互中锁定右下 resize 光标。任一 client 坐标轴位移严格超过 2px 后显示 resizing 视觉态；拖动期间临时 frame 在画布坐标中可缩至 2×2，节点整体 `opacity` 为 0.58。文字按临时宽度实时换行，绘制高度独立于临时 frame 高度，因此可越过边框且不被裁切。pointerup 时再恢复业务尺寸约束：先钳制到业务最小宽度，请求宽度大于文字自然宽度时收紧到自然宽度并设 `autoWidth: true`；否则保存请求宽度、允许换行并设为固定宽度，最终高度容纳全部文字。文字和 resize 的最终布局值都必须由实时层在 event 中给出，领域 apply 不读取 DOM 测量。
