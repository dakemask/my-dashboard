...
- 业务动作完成后自动保存到本机，支持上传、拉取，并在冲突时明确选择本地或云端完整资料库；
...
空资料库在资料库面板中显示创建提示。没有当前脑图时隐藏业务画布、编辑区保持空白并禁用画布命令；空脑图复位使用默认 viewport。最近脑图已不存在时清除偏好，不自动挑选另一张图。

### 资料库

- 文件夹可无限嵌套并直接包含文件夹和脑图。
- 单击文件夹同时选中并切换折叠；单击脑图同时选中并打开；单击资料库空白区域清空资料库选择。资料库选择与画布选择互相清除。
- 新建以当前选中文件夹为父目录；选中脑图时使用其父目录；没有适用选择时使用根目录。
- 新建、移动或历史定位会展开显示目标所需的祖先；恢复最近脑图严格保留原展开集合，不额外展开。
- 顶部操作只有新建文件夹、新建脑图、重命名和删除。
- 名称使用无感行内编辑：编辑器复用只读名称的行高、字号、内外边距和图标栅格，并保留选中态、当前脑图字重及未保存标记的占位，进入编辑时不能改变行高、名称可用宽度或树结构；只通过不占布局的内描边提示焦点。Enter、blur 或点击编辑器外区域在校验成功时提交；明确的取消按钮丢弃草稿。普通无效提交保留编辑器并显示错误；SDK settle 到来时无效草稿被取消。
- 项目只通过 HTML 拖放移动到现存文件夹或根目录；悬停折叠文件夹约 650 ms 自动展开。文件夹不能放入自身或后代。
- 删除脑图必须确认；删除文件夹必须明确提示递归删除所有后代后再确认。
- Delete 在资料库上下文打开同一删除确认，不直接删除。
- 相对本地保存基线有变化时，受影响脑图和现存祖先文件夹显示 `*`；自动保存成功后消失。
...
按下右下手柄后，画布在本次交互中锁定右下 resize 光标。任一 client 坐标轴位移严格超过 2px 后显示 resizing 视觉态；拖动期间临时 frame 在画布坐标中可缩至 2×2，节点整体 `opacity` 为 0.58。文字按临时宽度实时换行，绘制高度独立于临时 frame 高度，因此可越过边框且不被裁切。pointerup 时再恢复业务尺寸约束：先钳制到业务最小宽度；请求宽度大于文字自然宽度时收紧到自然宽度并设 `autoWidth: true`；否则允许换行并设为固定宽度，其中当前换行排版的右侧余量不足一个“字”的实测宽度时，无论 resize 前的 `autoWidth` 值为何，都收紧到保持当前换行所需的最小宽度，其余情况保存请求宽度。最终高度容纳全部文字。文字和 resize 的最终布局值都必须由实时层在 event 中给出，领域 apply 不读取 DOM 测量。
...
- 每个已完成的业务动作在 event 成功提交后立即尝试自动保存到本机；undo/redo 完成后同样自动保存。输入、拖动、resize 等实时过程不保存，直到形成 event。
- 顶栏没有常驻的手动保存命令。自动保存失败时保留当前暂存 payload 和撤销历史，持续显示未保存状态及“重试保存”按钮；重试成功后按钮消失。
- 上传按钮表示把当前本机版本同步到云端。
...
“精确 Ctrl”表示 Ctrl 按下且 Shift/Alt/Meta 均未按下。Ctrl 历史命令和 Alt+1/2 即使焦点在输入控件也执行模块命令。模块不接管 `Ctrl+S`。输入法 composition 期间不得误提交 Enter。
...
刚提交 event 后的自动保存只保存已经提交的 payload，`settle("local-save")` 返回 null，不结算或取消该 event 之后刚开始的实时交互（例如新增节点后立即开始的文字编辑）。保存失败后的用户重试、`upload`、`pull`、`remote-change`、`undo` 和 `redo` 使用同一结算规则：结束名称或文字草稿，取消资料库拖放、画布 pointer、连线与自动平移，并返回至多一个 event。
