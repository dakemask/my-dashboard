...
应用部署到 GitHub Pages。源代码保留在仓库中；GitHub Actions 运行工作流来安装依赖、将 Vite 输出构建到 `dist/`，并把生成的产物部署到 Pages。不要提交 `dist/`。
...
尽早保持模块解耦。当逻辑开始混合独立职责时，在它变成大文件问题之前就进行抽离。常见边界包括 UI 渲染、浏览器存储、远程 API 访问、数据规范化、纯领域操作和页面级编排。

做重要选择前先询问。如果存在有意义的选项、需求不清晰、用户可见行为变化、部署变化、公开路由变化、数据格式变化，或隐私/安全取舍，请在编辑前使用自定义用户输入工具。不要在这些情况下替用户静默选择。

除非用户明确要求改变，否则保留公开行为。现有路由、localStorage key、JSON 数据形状和部署假设都应视为兼容性表面。

优先使用小型、有类型的模块。TypeScript 类型应描述持久化数据、设置、API 响应和模块契约。除非是在验证未知外部数据的明确边界处，否则避免使用 `any`。

使用安全的 DOM 模式。通过 `textContent` 或显式 DOM 节点渲染用户提供的内容，而不是 HTML 字符串模板。

保持 UI 文案与应用一致。当前产品 UI 是中文，所以新的用户可见文案通常也应使用中文，除非用户另有要求。
...
这个模块允许用户快速记录短想法、附加可选标签、在已加载列表中本地搜索，并将 JSON 数据文件同步到私有 GitHub 数据仓库。

当前持久化行为：

- 设置使用现有的 `thought_*` localStorage key 存储在浏览器中。
- 数据文件默认是已配置私有仓库中的 `data/thoughts.json`。
- 浏览器使用用户提供的细粒度 GitHub token 读取和更新该 JSON 文件。
- 即使旧记录缺少 `tags` 或 `updatedAt` 等可选字段，也应保持可读。
...
私有用户数据不应存储在这个仪表盘仓库中。对于需要持久化的模块：

- 将数据作为 JSON 文件存储在单独的私有 GitHub 仓库中。
- 从浏览器通过 GitHub Contents API 访问这些文件。
- 使用限定到私有数据仓库、且拥有 Contents 读写权限的细粒度 token。
- token 只存储在用户的浏览器设置中。永远不要提交 token、将其烘焙进构建产物，或为了客户端使用把它放入 GitHub Actions secrets。
- 将 JSON 格式视为兼容性契约。如果格式需要改变，请先询问用户，并提供迁移路径或向后兼容的读取器。

这种模型通过分离公开静态应用资源和私有数据来提供隐私保护。它并不会把浏览器 localStorage 变成安全保险库；任何能访问用户浏览器配置文件的人都可能读到 token。
...
除非用户明确要求，否则不要启动 `npm run dev`。基于浏览器的运行时测试、布局审查和用户验收测试由用户负责。如果某项变更需要手动验证，请说明用户应检查什么，而不是自行启动开发服务器。
